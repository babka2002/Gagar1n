---
id: 7e926ef6-5439-429b-83d8-5e99376e6459
blueprint: page_personal_training
title: 'Персональные тренировки'
template: grelka/personal_training
updated_by: 9fcd11e1-673b-479c-8888-c690f9401ee3
updated_at: 1752494962
hero_title: персональные<br>тренировки
hero_image: img/individual_trainings_hero.jpg
features_list:
  -
    id: feat1
    feature_text: 'ИНДИВИДУАЛЬНЫЙ ПОДХОД'
    type: feature
    enabled: true
  -
    id: feat2
    feature_text: 'КОНТРОЛЬ ЗА ТЕХНИКОЙ ВЫПОЛНЕНИЯ УПРАЖНЕНИЙ'
    type: feature
    enabled: true
  -
    id: feat3
    feature_text: 'ПОДБОР ПРОГРАММЫ ПО ЦЕЛИ КЛИЕНТА'
    type: feature
    enabled: true
  -
    id: feat4
    feature_text: 'ПОСТОЯННАЯ МОТИВАЦИЯ И ПОДДЕРЖКА ТРЕНЕРА'
    type: feature
    enabled: true
  -
    id: feat5
    feature_text: 'ВЕСЬ НЕОБХОДИМЫЙ ИНВЕНТАРЬ'
    type: feature
    enabled: true
features_subtitle: 'У НАС ТЫ ЛЕГКО НАЙДЕШЬ СВОЙ ФОРМАТ И ПОДХОДЯЩЕГО ТРЕНЕРА'
trainers_title: 'НАШИ ТРЕНЕРЫ'
trainers_image: collage7photos.jpg
trainers_description: 'все ТРЕНЕРЫ В GAGAR1N имееют высокую квалификацию. Нам важно, чтобы здоровый образ жизни был доступен каждому: вне зависимости от возраста, положения и физической подготовки. С нами Вы становитесь динамичнее, позитивнее и даете пример своим детям!'
programs_title: 'НАШИ ПРОГРАММЫ'
programs_replicator:
  -
    id: prog1
    program_title: 'функциональные тренировки'
    program_image: img/functional_training.jpg
    program_description: 'Функциональные тренировки — это вид физической активности, направленный на развитие движений, которые человек использует в повседневной жизни: приседания, наклоны, подъёмы, повороты и перенос тяжестей. Эти тренировки укрепляют мышцы-стабилизаторы, улучшают баланс, координацию и гибкость, помогая повысить общую физическую форму и снизить риск травм в реальной жизни.'
    type: program
    enabled: true
  -
    id: prog2
    program_title: 'силовые тренировки'
    program_image: img/strength_training.jpg
    program_description: 'Силовые тренировки — это занятия, направленные на развитие мышечной силы и массы с помощью отягощений, таких как штанги, гантели, тренажёры или собственный вес тела. Они помогают укрепить мышцы, связки и суставы, ускоряют обмен веществ и улучшают общее состояние здоровья и физическую выносливость.'
    type: program
    enabled: true
  -
    id: prog3
    program_title: 'восстановительные программы'
    program_image: img/rehabilitation_programs.jpg
    program_description: 'Восстановительные программы — это комплекс тренировок и процедур, направленных на восстановление организма после физических нагрузок, травм или стрессов. Они включают лёгкие физические упражнения, растяжку, массаж, дыхательные практики и помогают улучшить подвижность, снять напряжение и ускорить процесс восстановления мышц и суставов.'
    type: program
    enabled: true
  -
    id: prog4
    program_title: 'восстановление после родов'
    program_image: img/postpartum_recovery.jpg
    program_description: 'Восстановление после родов — это комплекс мер, направленных на постепенное укрепление организма женщины после беременности и родов. Оно включает лёгкие физические упражнения для восстановления мышц тазового дна, пресса и спины, улучшение осанки, а также работу над общим тонусом и эмоциональным состоянием, с учётом индивидуальных особенностей и состояния здоровья.'
    type: program
    enabled: true
  -
    id: prog5
    program_title: 'ДЛЯ ДЕТЕЙ'
    program_image: img/FOR_CHILDREN.jpg
    program_description: 'Тренировки для детей — это специально разработанные занятия, направленные на развитие силы, координации, гибкости и выносливости с учётом возраста и особенностей растущего организма. Они проходят в игровой форме, помогают укрепить здоровье, улучшить осанку, развить уверенность в себе и привить любовь к физической активности с раннего возраста.'
    type: program
    enabled: true
  -
    id: prog6
    program_title: 'ДЛЯ СПОРТСМЕНОВ'
    program_image: img/FOR_ATHLETES.jpg
    program_description: 'Тренировки для спортсменов — это специализированные программы, направленные на развитие силы, скорости, выносливости, координации и других качеств, необходимых для конкретного вида спорта. Они учитывают уровень подготовки, цели и особенности спортсмена, помогают повысить результаты, предотвратить травмы и поддерживать оптимальную физическую форму.'
    type: program
    enabled: true
reviews_title: 'ОТЗЫВЫ КЛИЕНТОВ'
reviews_replicator:
  -
    id: rev1
    review_image: img/functional_training.jpg
    review_name: Имя
    review_subtitle: Имя
    review_text: 'Далеко-далеко за словесными горами в стране гласных и согласных живут рыбные тексты. Залетают большого щеке возвращайся! Повстречался, то свое. Заманивший лучше, первую образ до, подпоясал прямо курсивных рукописи осталось журчит снова большого!'
    type: review
    enabled: false
  -
    id: rev2
    review_image: img/functional_training.jpg
    review_name: Имя
    review_subtitle: Имя
    review_text: 'Далеко-далеко за словесными горами в стране гласных и согласных живут рыбные тексты. Залетают большого щеке возвращайся! Повстречался, то свое. Заманивший лучше, первую образ до, подпоясал прямо курсивных рукописи осталось журчит снова большого!'
    type: review
    enabled: false
  -
    id: rev3
    review_image: img/functional_training.jpg
    review_name: Имя
    review_subtitle: Имя
    review_text: 'Далеко-далеко за словесными горами в стране гласных и согласных живут рыбные тексты. Залетают большого щеке возвращайся! Повстречался, то свое. Заманивший лучше, первую образ до, подпоясал прямо курсивных рукописи осталось журчит снова большого!'
    type: review
    enabled: false
faq_title: 'ЧАСТО ЗАДАВАЕМЫЕ ВОПРОСЫ'
faq_replicator:
  -
    id: faq1
    question: 'Как выбрать подходящего тренера?'
    answer: 'Определите свои цели — похудение, набор массы, реабилитация или улучшение выносливости. Наши менеджеры помогут подобрать тренера, исходя из ваших задач, уровня подготовки и предпочтений.'
    type: faq
    enabled: true
  -
    id: faq2
    question: 'Какие результаты можно ожидать?'
    answer: 'Первые заметные изменения вы почувствуете уже через 4–6 недель регулярных занятий. Итог зависит от ваших целей, регулярности тренировок и соблюдения рекомендаций тренера.'
    type: faq
    enabled: true
  -
    id: faq3
    question: 'Нужно ли иметь специальную форму для тренировок?'
    answer: 'Да, для вашего комфорта и безопасности рекомендуется спортивная одежда и обувь с нескользящей подошвой.'
    type: faq
    enabled: true
  -
    id: faq4
    question: 'Что делать, если я новичок в фитнесе?'
    answer: 'Не переживайте! Мы предложим вводное занятие, поможем составить индивидуальный план и адаптировать тренировки под ваш уровень. Все начинается с малого — главное начать!'
    type: faq
    enabled: true
  -
    id: md328n79
    question: 'Сколько длится тренировка?'
    answer: |-
      В тренажёрном зале и Групповых программах - 55 минут, в Аква зоне - 45 минут
      Это оптимальное время для эффективной разминки, основной части и заминки.
    type: faq
    enabled: true
training_title: 'не знаешь как начать?'
training_image: img/trainer-hero.jpg
training_text: 'запишись на индивидуальную тренировку'
form_title: 'НЕ УПУСТИ ЭТУ ВОЗМОЖНОСТЬ'
form_image: img/pool-contact.png
field_meta_description: 'Персональные тренировки'
field_meta_keywords: 'Персональные тренировки'
---
